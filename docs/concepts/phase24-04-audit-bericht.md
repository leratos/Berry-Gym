...
**Vorgehen:** Read-only Django-Shell-Query auf Production-DB (User `[redacted]`, Skript `/tmp/phase24_4_audit.py`)
...
```bash
ssh [redacted] 'cd gym.last-strawberry.com && source venv/bin/activate && python manage.py shell < /tmp/phase24_4_audit.py'
```
