...
def find_substitute_exercise(original_name, required_equipment, available_equipment):
    ...
    logger.info(
        f"Finding substitute for: {original_name}, required: {required_equipment}, available: {available_equipment}"
    )
        ...
        if not original_uebung:
            ...
            for key, muscle in exercise_to_muscle.items():
                if key in original_name.lower():
                    ...
                    original_uebung = type(
                        "obj",
                        (object,),
                        {"muskelgruppe": muscle, "bewegungstyp": "ISOLATION", "id": -1},
                    )()
                    break
            ...
            if muscle_group and len(available_equipment_objects) > 0:
                ...
                if band_eq and band_eq in available_equipment_objects:
                    band_exercise = (
                        ...
                        .exclude(id=original_id if original_id > 0 else 0)
                        .first()
                    )
                ...
                for equip_obj in available_equipment_objects:
                    similar = (
                        Uebung.objects.filter(muskelgruppe=muscle_group, equipment=equip_obj)
                        .exclude(id=original_id if original_id > 0 else 0)
                        .first()
                    )
