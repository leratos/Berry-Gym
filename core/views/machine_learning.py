    ...
    try:
        ...
        if request.method == "POST" and request.body:
            ...
            try:
                ...
                return JsonResponse(
                    {"success": False, "message": "Ungültige JSON-Daten"}, status=400
                )
...
def ml_model_info(request: HttpRequest, uebung_id: int) -> JsonResponse:
    ...
    try:
        ...
        if info:
            return JsonResponse({"success": True, "uebung": uebung.bezeichnung, "model_info": info})
        else:
            return JsonResponse(
                {"success": False, "message": "Kein Modell für diese Übung trainiert"}, status=404
            )
...
def ml_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    models = (
        MLPredictionModel.objects.filter(user=request.user)
