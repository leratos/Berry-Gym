        ...
        return JsonResponse(
            {
                ...
                "message": (
                    ...
                    if created
                    else "Push-Benachrichtigungen aktualisiert"
                ),
            }
        )
...
def unsubscribe_push(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": f"{deleted_count} Subscription(s) gelöscht"}
        )
        ...
        return JsonResponse(
            {
                "error": "An internal error has occurred while unsubscribing from push notifications."
            },
            status=500,
        )
...
def get_vapid_public_key(request: HttpRequest) -> JsonResponse:
    ...
    return JsonResponse({"publicKey": public_key_base64})
