...
def validate_and_fix_json():
    ...
    for i, exercise in enumerate(exercises):
        for field in required_fields:
            if field not in exercise:
                errors.append(f"[FEHLER] Index {i} ({exercise.get('bezeichnung', 'UNBEKANNT')}): Fehlendes Feld '{field}'")

        # Prüfe ob hilfsmuskeln ein Array ist
        if 'hilfsmuskeln' in exercise and not isinstance(exercise['hilfsmuskeln'], list):
            errors.append(f"[FEHLER] Index {i} ({exercise.get('bezeichnung')}): 'hilfsmuskeln' ist kein Array")
        ...
        if 'equipment' in exercise and not isinstance(exercise['equipment'], list):
            errors.append(f"[FEHLER] Index {i} ({exercise.get('bezeichnung')}): 'equipment' ist kein Array")
            exercise['equipment'] = []
    ...
    for i, exercise in enumerate(exercises):
        ...
        has_standards = all(k in exercise for k in ['standard_beginner', 'standard_intermediate', 'standard_advanced', 'standard_elite'])
        ...
        if has_standards:
            ...
            if not (beg <= inter <= adv <= elite):
                errors.append(f"[FEHLER] Index {i} ({bezeichnung}): Standards nicht aufsteigend: {beg}/{inter}/{adv}/{elite}")
            ...
            if gewichts_typ in ['ZEIT', 'KOERPERGEWICHT']:
                ...
                if bewegungstyp in ['CARDIO', 'KOMPLEX'] or 'Plank' in bezeichnung or 'Crunch' in bezeichnung:
                    warnings.append(f"[WARNUNG] Index {i} ({bezeichnung}): Hat Standards aber ist {gewichts_typ}/{bewegungstyp}")
        else:
            ...
            if gewichts_typ not in ['ZEIT', 'KOERPERGEWICHT']:
                ...
                if 'Band' not in bezeichnung and 'Latziehen (Widerstandsband)' not in bezeichnung:
                    warnings.append(f"[WARNUNG] Index {i} ({bezeichnung}): Keine Standards aber hat Gewichte ({gewichts_typ})")
