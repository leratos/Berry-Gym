...
from core.models import KoerperWerte, Plan, Satz, Trainingseinheit, Uebung
...
class LiveGuidanceTester:
    ...
    def __init__(self, user_id: int = 1, use_openrouter: bool = False):
        ...
        try:
            ...
            self.user = User.objects.create_user(
                username=f"test_user_{user_id}", password="testpass123"
            )
    ...
    def setup_test_data(self):
        ...
        koerperwert = KoerperWerte.objects.create(
            user=self.user, gewicht=80.5, groesse_cm=180, koerperfett_prozent=15.0
        )
        ...
        self.test_sessions["empty"] = Trainingseinheit.objects.create(
            user=self.user, plan=plan, datum=timezone.now()
        )
        ...
        self.test_sessions["warmup_only"] = Trainingseinheit.objects.create(
            user=self.user, plan=None, datum=timezone.now() - timedelta(minutes=10)  # Kein Plan
        )
        if uebung1:
            Satz.objects.create(
        ...
        print(
            f"✓ Test-Session 'warmup_only': ID {self.test_sessions['warmup_only'].id} (nur Warm-up)"
        )
        ...
        print(
            f"✓ Test-Session 'no_rpe': ID {self.test_sessions['no_rpe'].id} (2 Übungen, keine RPE)"
        )
    ...
    def test_scenario(self, name: str, session_key: str, question: str, exercise_id=None):
        ...
        try:
            ...
            self.test_results.append({"name": name, "status": "fail", "message": str(e)})
            return False
    ...
    def run_all_tests(self):
        ...
        self.test_scenario(
            "Normale Session - Satzzahl", "normal", "Wie viele Sätze soll ich noch machen?"
        )

        # TEST 2: Leere Session (keine Sätze)
        self.test_scenario(
            "Leere Session - Warm-up Frage", "empty", "Wie soll ich mit dem Warm-up anfangen?"
        )
        ...
        self.test_scenario(
            "Keine RPE - RPE-Empfehlung", "no_rpe", "Welche RPE sollte ich anstreben?"
        )

        # TEST 5: Sehr kurze Frage
        self.test_scenario("Kurze Frage", "normal", "Pause?")
        ...
        self.test_scenario("Sonderzeichen", "normal", "Soll ich @home mit 50kg/bar trainieren? 🏋️")
        ...
        self.test_scenario("Off-Topic Ernährung", "normal", "Was soll ich heute Abend essen?")

        # TEST 10: Motivations-Frage
        self.test_scenario(
            "Motivation", "normal", "Ich bin heute nicht motiviert. Was kann ich tun?"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use-openrouter", action="store_true", help="OpenRouter nutzen (kostet Geld!)"
    )
