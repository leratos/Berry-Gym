...
class DatabaseClient:
    ...
    def start_tunnel(self):
        ...
        ssh_cmd = [
            ...
            '-L', f'{ai_config.LOCAL_BIND_PORT}:{ai_config.DB_HOST}:{ai_config.DB_PORT}',
            f'{ai_config.SSH_USERNAME}@{ai_config.SSH_HOST}',
            '-p', str(ai_config.SSH_PORT)
        ]
        ...
        try:
            ...
            print(f"✓ SSH Tunnel aktiv: localhost:{ai_config.LOCAL_BIND_PORT} → {ai_config.SSH_HOST}:{ai_config.DB_PORT}")
            print(f"   PID: {self.tunnel_process.pid}")
            
        except FileNotFoundError:
            raise RuntimeError("SSH command nicht gefunden. Ist OpenSSH installiert?\n" +
