...
class TrainingAnalyzer:
    ...
    MUSKELGRUPPEN = [
        ...
        'Bauch', 'Unterer Rücken', 'Waden', 'Unterarme', 'Nacken',
        'Trapez', 'Vordere Schulter', 'Seitliche Schulter', 'Hintere Schulter',
        'Oberer Rücken', 'Mittlerer Rücken', 'Latissimus',
        'Oberschenkel Vorne', 'Oberschenkel Hinten', 'Gesäß', 'Hüfte'
    ]
    
    # Push/Pull Kategorisierung
    PUSH_GROUPS = ['Brust', 'Schultern', 'Trizeps', 'Vordere Schulter', 'Seitliche Schulter']
    PULL_GROUPS = ['Rücken', 'Bizeps', 'Trapez', 'Oberer Rücken', 'Mittlerer Rücken', 'Latissimus', 'Hintere Schulter']
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        # Daten sammeln
        muscle_volume = defaultdict(lambda: {'effective_reps': 0, 'avg_rpe': [], 'last_trained': None})
        ...
        for session in sessions:
            for satz in session.saetze.select_related('uebung').filter(ist_aufwaermsatz=False):
                ...
                if not muscle_volume[mg]['last_trained'] or session.datum > muscle_volume[mg]['last_trained']:
        ...
        for mg in muscle_volume:
            if muscle_volume[mg]['avg_rpe']:
                muscle_volume[mg]['avg_rpe'] = round(sum(muscle_volume[mg]['avg_rpe']) / len(muscle_volume[mg]['avg_rpe']), 1)
        ...
        for ex_name in exercise_performance:
            ...
            if len(records) >= 2:
                ...
                exercise_performance[ex_name]['trend'] = f"+{trend}kg" if trend > 0 else f"{trend}kg"
                exercise_performance[ex_name]['last_1rm'] = last_1rm
                exercise_performance[ex_name]['avg_rpe'] = round(sum(r['rpe'] for r in records) / len(records), 1)
            else:
        ...
        # Push/Pull Balance
        push_volume = sum(muscle_volume[mg]['effective_reps'] for mg in self.PUSH_GROUPS if mg in muscle_volume)
        pull_volume = sum(muscle_volume[mg]['effective_reps'] for mg in self.PULL_GROUPS if mg in muscle_volume)
        ...
        return {
            ...
            'training_stats': {
                ...
                'avg_duration_minutes': round(total_duration / total_sessions) if total_sessions > 0 else 0,
            ...
            'push_pull_balance': {
                ...
                'balanced': 0.85 <= (push_volume / pull_volume) <= 1.15 if pull_volume > 0 else False
            },
    ...
    def _identify_weaknesses(self, muscle_volume: dict, exercise_performance: dict) -> List[str]:
        ...
        if muscle_volume:
            avg_volume = sum(mg['effective_reps'] for mg in muscle_volume.values()) / len(muscle_volume)
            
            # Muskelgruppen mit <60% des Durchschnitts
            for mg, data in muscle_volume.items():
                if data['effective_reps'] < avg_volume * 0.6:
                    weaknesses.append(f"{mg}: Untertrainiert (nur {round(data['effective_reps'])} eff. Wdh vs. Ø {round(avg_volume)})")
    ...
    def print_summary(self, analysis: Dict[str, Any] = None):
        ...
        sorted_mg = sorted(analysis['muscle_groups'].items(), 
        ...
        sorted_ex = sorted(analysis['exercise_performance'].items(),
        ...
        print(f"   Ratio: {balance['ratio']} {'✓ Balanced' if balance['balanced'] else '✗ Unbalanced'}")
